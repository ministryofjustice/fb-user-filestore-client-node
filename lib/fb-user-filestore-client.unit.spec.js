...
test('When requesting a user file with a valid fingerprint', async t => {
  ...
  t.equal(callArgs[0], '/service/:serviceSlug/user/:userId/:fingerprint', 'it should pass the correct url pattern')
  t.deepEqual(callArgs[1], {serviceSlug: 'testServiceSlug', userId: 'testUserId', fingerprint: '31-x9323s39amdsa'}, 'it should add the x-access-token header')
...
test('When storing a user file', async t => {
  ...
  t.equal(callArgs[0], '/service/:serviceSlug/user/:userId', 'it should pass the correct url pattern')
  t.deepEqual(callArgs[1], {serviceSlug: 'testServiceSlug', userId: 'testUserId'}, 'it should pass the correct context to substitue keys in the url')
  t.deepEqual(callArgs[2], {
  ...
  const policyBundle = sendPostStub.getCall(0).args[2].policy
  ...
  const policyBundleB = sendPostStub.getCall(0).args[2].policy
